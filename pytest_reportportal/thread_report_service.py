...
class DelayedReportHandler(threading.Thread, metaclass=Singleton):
    """Pytest service class for reporting test results to the Report Portal."""

    def __init__(self):
        super(DelayedReportHandler, self).__init__()
        self.done = False
        self.name = "Report Thread"
        ...
        self.report_queue: queue.Queue = None
    ...
    def _add_item_hier_parts_dirs(item, hier_flag, dirs_level, report_parts,
        ...
        :return: str rp_name
        """
        parts_dirs = DelayedReportHandler._get_item_dirs(item)
    ...
    def run(self) -> None:
        while True:
            if not self.report_queue.empty():
                ...
                start_pytest_item_id = self.start_pytest_item(report_data.item)
                if report_data.logs:
                    for message, level, attachment in report_data.logs:
                        self.post_log(message, level, attachment)
                self.finish_pytest_item(report_data.item, start_pytest_item_id, report_data.result, report_data.issue)


class DelayedReportData(object):
    ...
    def add_log(self, data):
        """Adding log to Logs sequence"""
        self.logs.append(data)

    def add_result(self, result, issue):
        self.result = result
        self.issue = issue
